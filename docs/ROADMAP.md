...
> 🧭 개인화(인테이크 재설계 + 유저 프로파일)의 단계 전략은 [PERSONALIZATION.md](PERSONALIZATION.md)에 별도 정리. 인테이크는 대화형(방향 B)으로 가며, Phase 1(대화형 인테이크)은 백엔드 없이 지금 가능.
> 💡 떠오르는 아이디어는 [IDEAS.md](IDEAS.md)에 계속 쌓는다(살아있는 브레인스토밍). 실행 단계 오면 여기 ROADMAP으로 승격.
