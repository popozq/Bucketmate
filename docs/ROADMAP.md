...
**브랜드 / 상표 (⚠️ 이미 동명 서비스 존재 — bucketmate.kr / 운영사 이노플)**
- 상황: bucketmate.kr이 "버킷리스트 모임·동행 매칭" 서비스로 운영 중(같은 이름, 다른 분야/도메인). 우리는 AI 에이전트 플랫폼이라 분야가 다름.
- 핵심: 한국은 **선출원주의**(먼저 출원한 사람 우선). 다만 ① 상품/서비스 **류(類)**가 다르면 공존 가능, ② 이름 동일 → **혼동 가능성** 거절사유 될 수 있음, ③ 저쪽이 주지·저명하면 **부정경쟁방지법** 리스크, ④ 존재를 알고 하는 출원은 **"부정한 목적"**(상표법 제34조)으로 보일 수 있어 주의.
- [ ] **KIPRIS**(kipris.or.kr)에서 "버킷메이트"·"Bucketmate" 전 류 검색 — 이노플 등록/출원 여부 및 류 확인 (9·35·41·42·45류 위주)
- [ ] **변리사 상담** — 선행상표 조사 + 류 선정(우리는 9류 SW / 42류 SaaS 유력) + 출원 대행
- [ ] 최초 사용일 증거 보관 (git 첫 커밋 2026-06-22 = 독자적 선의 사용 근거)
- [ ] 대체 브랜드명 후보 1개 준비 (클리어 전엔 브랜딩 과투자 금지)
- 목표: 대한민국에 "Bucketmate" + "버킷메이트" 상표 출원 (사용자가 사전 조사 시 동일/유사 미발견)
